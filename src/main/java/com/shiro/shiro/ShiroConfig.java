...
public class ShiroConfig {
     ...
     */
    @Bean                                                //@Qualifier("securityManager")  与注入@Bean("securityManager") 名称一致
    public ShiroFilterFactoryBean getSiroFilterFactoryBean(@Qualifier("securityManager") DefaultWebSecurityManager securityManager) {
        ...
        //设置未登录跳转的登录页面
        shiroFilterFactoryBean.setLoginUrl("/login/loginView");
        //设置未授权页面
     ...
     */
    @Bean(name = "securityManager")                //@Qualifier("UserRealm")  通过这个注解 找到下面的UserRealm
